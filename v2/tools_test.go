...
func TestTools_PushJSONToRemote(t *testing.T) {
	...
	if err != nil {
		t.Error("failed to call romote url:", err)
	}
}
...
var jsonTests = []struct {
	...
	{name: "Badly formatted json", json: `{"foo":}`, errorExpected: true, maxSize: 1024, allowUnknow: false},
	{name: "incorrect type", json: `{"foo": 1}`, errorExpected: true, maxSize: 1024, allowUnknow: false},
	{name: "two json files", json: `{"foo": "bar"}{"alpha": "beta"}`, errorExpected: true, maxSize: 1024, allowUnknow: false},
	{name: "empty body", json: ``, errorExpected: true, maxSize: 1024, allowUnknow: false},
	{name: "syntax error in json", json: `{"foo": bar"}`, errorExpected: true, maxSize: 1024, allowUnknow: false},
	{name: "unknown field in json", json: `{"fooo": "1"}`, errorExpected: true, maxSize: 1024, allowUnknow: false},
	{name: "allow unknown fields in json", json: `{"fooo": "1"}`, errorExpected: false, maxSize: 1024, allowUnknow: true},
	{name: "missing field name", json: `{jack: "1"}`, errorExpected: true, maxSize: 1024, allowUnknow: true},
	{name: "file too large", json: `{"foo": "bar"}`, errorExpected: true, maxSize: 5, allowUnknow: true},
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests{
		...
		testTool.AllowUnknownFields = e.allowUnknow

		// decalare a variable to read the decoded json into
