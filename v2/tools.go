...
const randomStringSource = "abcdefghijklmnopqrstuvwxyzABACDEFGHIJKLMNOPQRSTUVWXYZ0123456789_+"

// Tools is the type used to instantiate this module. Any variable of this type will have access 
// to allthe methods with the reciever *Tools
type Tools struct {
	MaxFileSize 		int
...
// RandomString returns a string of random characters of len n, using randomStringSource
	...
	for _, FHeaders := range r.MultipartForm.File {
		for _, hdr := range FHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				if !allowed {
					return nil, errors.New("the uploaded file is not pemitted")
				}
...
// in the browser window by setting content disposition. it also allows sepcification of the 
// display name
...
// PushJSONToRemote posts aribtary data to some URI as JSON, and returns the responce, status code, and error if any
